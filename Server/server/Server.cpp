...
SOCKET Server::initServer()
{
	...
	if (_serverSock == INVALID_SOCKET)
	{
		CELLLOG_INFO("<socket=%d> socket error.", (int)_serverSock);
	}
	else
	{
		//CELLLOG_INFO("<socket=%d> socket success.",(int)_server_sock);
	}
	return _serverSock;
}
...
int Server::Bind(const char* ip, unsigned short port)
{
	...
	if (ret == SOCKET_ERROR)
	{
		CELLLOG_INFO("<socket=%d> bind error.", (int)_serverSock);
	}
	else
	{
		//CELLLOG_INFO("<socket=%d> bind success.", (int)_server_sock);
	}
	return ret;
}
...
int Server::Listen(int n)
{
	...
	if (ret == SOCKET_ERROR)
	{
		CELLLOG_INFO("<socket=%d> listen error.", (int)_serverSock);
	}
	else
	{
		//CELLLOG_INFO("<socket=%d> listen success.", (int)_server_sock);
	}
	return ret;
}
...
SOCKET Server::Accept()
{
	...
	{
		CELLLOG_INFO("<socket=%d> accept error.", (int)_serverSock);
	}
	else
	{
		//向所有的客户端发送新加入的客户端
		...
		//CELLLOG_INFO("New client %d join.", (int)client_sock);
	}
	return client_sock;
}
